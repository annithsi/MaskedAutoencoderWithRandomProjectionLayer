...
def main():
  ...
  x_normal_masked, x_normal_projected = data_f.mask_and_return_projection(x_normal, n_components=3)
  x_anomaly_masked, x_anomaly_projected = data_f.mask_and_return_projection(x_anomaly, n_components=3)

  # Create dataloaders and model followed by training.
  dataloaders = data_f.get_loaders(x_normal_masked, x_normal_projected)
  model = models.DimReduce()
  ...
  err_normal = abs(predictions_normal - x_normal_projected)
  predictions_anomaly = models.evaluate(model, x_anomaly_masked)
  err_anomaly = abs(predictions_anomaly - x_anomaly_projected)
